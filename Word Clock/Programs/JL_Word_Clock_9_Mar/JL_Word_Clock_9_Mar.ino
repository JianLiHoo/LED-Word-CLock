 ...
 if(settingMode==true){
    ...
    switch (settingsMenu){
        ...
        case 4:
          ...
          blinkWord(arrFOURH, colorWhite, 3);
          break;
    ...
    while(digitalRead(SETPIN)==HIGH){  //not pressed
      ...
      switch (settingsMenu){
        ...
        case 3: //Date Setting. Year, Month, Day.
        
          //Max Years: 2100. Must Check DS1307 datasheet.
          //Max Months: 12
          //Max Days: 31
          ...
          //Leap Years: 2012, 2016, 2020, 2024, 2028,...

          Serial.println("Date Setting, Year");
          displayNum(setYear, colorGreen);
          ...
          while(digitalRead(SETPIN)==HIGH){  //Not pressed
            if(digitalRead(LEFTPIN)==LOW){
              ...
              if(setYear<2015){
                setYear = 2099;  //Max Number of Years
              }
              else{
                setYear = setYear - 1;
              }
            ...
            if(setYear==now.year()){
              ...
              displayYear(setYear);
              rtc.adjust(DateTime(setYear,now.month(), now.day(), now.hour(), now.minute(), now.second()));  //reset seconds to 0 as well
          ...
          Serial.println("Date Setting, Month");
          displayNum(setMonth, colorBlue);
          ...
          while(digitalRead(SETPIN)==HIGH){  //Not pressed
            if(digitalRead(LEFTPIN)==LOW){
              ...
              if(setMonth<2){
                setMonth = 12;  //Max Number of Months
              }
              else{
                setMonth = setMonth - 1;
              }
            ...
            /*
            displayMonth(setMonth);
            rtc.adjust(DateTime(now.year(),setMonth, now.day(), now.hour(), now.minute(), now.second()));  //reset seconds to 0 as well
            Serial.print("setMonth = ");
            Serial.println(setMonth,DEC);
            delay(200);
            */
            
            if(setMonth!=now.month()){
              displayMonth(setMonth);
              rtc.adjust(DateTime(now.year(),setMonth, now.day(), now.hour(), now.minute(), now.second()));  //reset seconds to 0 as well
              Serial.print("setMonth = ");
              Serial.println(setMonth,DEC);
        ...
        case 4: //Birthday Setting
          Serial.println("Birthday Setting");
  ...
  if (now.hour()>=19){
    grid.setBrightness(nightBrightness* (pctBrightness/100));
    grid.show();
  }
  else{
    float brightness = dayBrightness * (pctBrightness/100);  //Percentage of day brightness
    grid.setBrightness(brightness);
    grid.show();
  }
  
  displayTime();
  checkForBirthday(31,3); //checkForBirthday(D,M)
  ...
  if(timeCount >20000){
    ...
    colorWipe(colorBlack,0);
    /*
    paintWord(arrsYear,colorWhite);
    displayNum(now.year(),colorBlue);
    delay(5000);
    colorWipe(colorBlack,0);
    */
    timeCount = 0;
...
void displayYear(int setYear){
  
    DateTime now = rtc.now();
  
    colorWipe(colorBlack,0);
    paintWord(arrsYear,colorWhite);
    displayNum(now.year(),colorGreen);
    
}
void displayMonth(int setMonth){
  
    DateTime now = rtc.now();
  
    colorWipe(colorBlack,0);
    paintWord(arrsMonth,colorWhite);
    displayNum(now.month(),colorGreen);
